...
namespace slox_plugin_backup\Util;
...
class DebugLog
{
    ...
    public function sendLog(string $message, $context)
    {
        ...
        if (((bool) $this->systemConfigService->get('slox_plugin_backup.config.DebugPost')) && $this->systemConfigService->get('slox_plugin_backup.config.DebugPostURL')) {
            $url = strval($this->systemConfigService->get('slox_plugin_backup.config.DebugPostURL'));
    ...
    public function sendLog_forwardRequestCopy(string $message, $request)
    {
        ...
        if (((bool) $this->systemConfigService->get('slox_plugin_backup.config.DebugPost')) && $this->systemConfigService->get('slox_plugin_backup.config.DebugPostURL')) {
            ...
            $requestGuzz = new Request(
                'POST',
                $this->systemConfigService->get('slox_plugin_backup.config.DebugPostURL'),
                ['Content-Type' => 'application/json'],
